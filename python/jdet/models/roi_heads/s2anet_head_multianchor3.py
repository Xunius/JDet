...
class S2ANetHead(nn.Module):
    ...
    def forward_single_train(self, x, stride, targets=None):
        ...
        for ii in range(self.n_anchors):
            ...
            all_odm_cls_score.append(odm_cls_score)
            all_odm_bbox_pred.append(odm_bbox_pred)
            refine_anchor.append(refine_anchorii)

        odm_cls_score = jt.stack(all_odm_cls_score, 1) # [B, n_anchors, 256, 128, 128]
        odm_bbox_pred = jt.stack(all_odm_bbox_pred, 1) # [B, n_anchors, 256, 128, 128]
        ...
        odm_bbox_pred = odm_bbox_pred.permute(0, 3, 4, 1, 2).reshape(B, -1, 5)
        odm_cls_score = odm_cls_score.permute(0, 3, 4, 1, 2).reshape(B, -1, label_channels)
    ...
    def forward_single_val(self, x, stride):
        ...
        odm_cls_score = jt.stack(all_odm_cls_score, 1) # [B, n_anchors, 256, 128, 128]
        odm_bbox_pred = jt.stack(all_odm_bbox_pred, 1) # [B, n_anchors, 256, 128, 128]
        odm_cls_score = odm_cls_score.reshape(B, -1, *featmap_size)
        odm_bbox_pred = odm_bbox_pred.reshape(B, -1, *featmap_size)
        refine_anchor = jt.stack(refine_anchor, 3) # [B, 128, 128, n_anchors, 5]
        refine_anchor = refine_anchor.reshape(B, -1, 5)
